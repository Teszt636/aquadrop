...
import { addContactsToGroups, createGroup, listGroups } from '@/lib/admin/b2b-email-store';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const row = await createGroup(body);
    await addContactsToGroups(body.contactIds, [row.id]);
